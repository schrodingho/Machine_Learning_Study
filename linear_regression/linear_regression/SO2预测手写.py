...
def train(x_train,y_train,epoch):
    ...
    for i in range(epoch):
        ...
        w_g = np.zeros(9)
        for j in range(3200):
            b_g += (y_train[j] - weights.dot(x_train[j, 12, :]) - bias) * (-1)
            for k in range(9):
                w_g[k] += (y_train[j] - weights.dot(x_train[j, 12, :]) - bias) * (-x_train[j, 12, k])
        b_g/=3200
        w_g/=3200
        ...
        if i%200==0:
            loss=0
            for j in range(3200):
                loss+=(y_train[j]-weights.dot(x_train[j,12,:])-bias)**2
            print(f"after {i} epochs, the loss on train data is {loss/3200}.")
            train_loss_list.append(loss/3200)
            epoch_list.append(i)
...
def validation(x_val,y_val,weights,bias):
    loss=0
    for i in range(400):
        loss+=(y_val[i]-weights.dot(x_val[i,12,:])-bias)**2
    return loss/400
...
if __name__=='__main__':
    ...
    plt.scatter(epoch_list,train_loss_list)
